              ...
              {installers.map(worker => (
                <div key={worker._id} 
                     ...
                     className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition ${
                       formData.selectedInstallers.includes(worker._id) ? 'bg-blue-600/20 border border-blue-500/50' : 'hover:bg-slate-700'
                     }`}>
                  <div className={`w-4 h-4 rounded border flex items-center justify-center ${
                    formData.selectedInstallers.includes(worker._id) ? 'bg-blue-500 border-blue-500' : 'border-slate-500'
                  }`}>
